...
```{r github-issues, fig.asp=1/(2*golden_ratio())}
issues <- con |>
  dbGetQuery("select count(*) issue_count, 
...
ggplot(issues, aes(as.Date(month), issue_count, color=type)) +
  labs(
    x = "month",
    y = "count",
    colors = '',
    title = "GitHub issues, pull requests and discussions on the OpenSSL reposistory"
  ) +
  geom_point() +
  #geom_label(check_overlap = TRUE) +
  scale_x_date(date_breaks = "4 year", 
               date_labels = "%Y", 
               limits= as.Date(ISOdate(c(2013, 2026),1,1))) + 
  theme_slides() +
...
```{r github-comments}
ggplot(issues, aes(as.Date(month), comments, color=type)) +
  labs(
    x = "month",
    y = "comments",
    colors = '',
    title = "GitHub comments on the OpenSSL Library repository"
  ) +
  geom_point() +
  #geom_label(check_overlap = TRUE) +
  scale_x_date(date_breaks = "4 year", 
